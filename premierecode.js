...
let results = document.getElementById('result'); //Finds the element with id result in the html file.
...
createExam(questionBank, exam, results, checkResult); 

function createExam(questions, quiz, result, check){
    function showQuest(questions, quiz){
        ...
        quiz.innerHTML = output.join('<br>'); //Seperates all objects in the array output with a <br> and then returns them so they show up on the premiere.html page.
    }
    function showResult(questions, quiz, result){
        ...
        if ((ansCorrect/questions.length) <= 6){ //If the user gets less than 60% questions correct, they fail.
            pass = ' You failed.';
        }
        else{
            pass = ' You passed!';
    ...
    showQuest(questions, quiz); //Calls the showQuest function (equivalent of methods in Java).
    check.onclick = function(){showResult(questions, quiz, result);}; //When the button with the id check is clicked, the user is given how many of the questions they got right.
}
